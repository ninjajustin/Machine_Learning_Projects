#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Sourced From: https://github.com/pulkitmaloo/Ultimate-Tic-Tac-Toe
...
def min_turn(state, last_move, player, depth, s_time, alpha, beta):
    global box_won
    #change depth to adjust for how far the ai can see
    #make a second ai instead of the player with different depth to see who wins more
...
#Function that manages console outputs to prompt input from user
...
#Function to run game with player vs AI
def player_game(state="." * 81, depth=5):
...
def game(state="." * 81, depth=5):
    ...
    #change starting board position
    bot_move_2 = 0
    bot_state_2 = add_piece(state, bot_move_2, "X")
    print("Bot 2 placed 'X' on", bot_move_2, "\n")
    print_board(bot_state_2)

    while True:
        print("Please wait, Bot 1 is thinking...")
        s_time = time()
        try:
            bot_state_1, bot_move_1 = minimax(bot_state_2, bot_move_2, "O", depth, s_time)
        except TypeError:
            game_won = "X"
            break

        print("#" * 40)
        print("Bot 1 placed 'O' on", bot_move_1, "\n")
        print_board(bot_state_1)
        state = bot_state_1
        box_won = update_box_won(bot_state_1)
        game_won = check_small_box(box_won)
        if game_won != ".":
            break

        #Bot 2
        #bot_move and bot_board do not match
        #ex - 20 goes to space 8 instead
        print("Please wait, Bot 2 is thinking...")
        s_time = time()
        try:
            bot_state_2, bot_move_2 = minimax(bot_state_1, bot_move_1, "X", depth, s_time)
        #no scenario where bot 2 wins --> forfeit
        except TypeError:
            game_won = "O"
            break
        ...
        print_board(bot_state_2)
        state = bot_state_2
        box_won = update_box_won(bot_state_2)
        game_won = check_small_box(box_won)
        if game_won != ".":
            break
    ...
    if game_won == "O":
        #print("$$$$$ Congratulations YOU WIN! $$$$$")
        print("$$$$$ Congratulations BOT 1 WINS! $$$$$")
    else:
        print("!!!!! Congratulations BOT 2 WINS! !!!!!")

    return state

#Function to simulate multiple games
def gameRepeat(state="." * 81, depth=5, start=0):
    global box_won, possible_goals
    possible_goals = [(0, 4, 8), (2, 4, 6)]
    possible_goals += [(i, i+3, i+6) for i in range(3)]
    possible_goals += [(3*i, 3*i+1, 3*i+2) for i in range(3)]
    ...
    while True:
        ...
        s_time = time()
        try:
            bot_state_1, bot_move_1 = minimax(
                bot_state_2, bot_move_2, "O", 1, s_time)
        except TypeError:
            game_won = "X"
            break
        ...
        s_time = time()
        try:
            bot_state_2, bot_move_2 = minimax(
                bot_state_1, bot_move_1, "X", 3, s_time)
        #no scenario where bot 2 wins --> forfeit
        except TypeError:
            game_won = "O"
            break
...
if __name__ == "__main__":
    ...
    #Uncomment code below to run repeated simulations
    # for i in range(81):
    #     final_state = gameRepeat(INITIAL_STATE, depth=5, start=i)
    #     if(final_state == 0):
    #         bot1wins.append(i)
    #     else:
    #         bot2wins.append(i)
    ...
    final_state = game(INITIAL_STATE, depth=4)
